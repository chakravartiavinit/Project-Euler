...
/*###############################################################################################################################################*/
void permute(string &a, int l, int r, vector<string> &vt)
{
    if (l == r)
        vt.push_back(a);
    else
...
int answer(int a, int b, int c, string &d)
{
    return (d[a] - '0') * 100 + (d[b] - '0') * 10 + (d[c] - '0');
}
bool check(string &a)
{
    int first = answer(1, 2, 3, a);
    int second = answer(2, 3, 4, a);
    int third = answer(3, 4, 5, a);
    int fourth = answer(4, 5, 6, a);
    int fifth = answer(5, 6, 7, a);
    int sixth = answer(6, 7, 8, a);
    int seventh = answer(7, 8, 9, a);
    if (first % 2 != 0 || second % 3 != 0 || third % 5 != 0 || fourth % 7 != 0 || fifth % 11 != 0 || sixth % 13 != 0 || seventh % 17 != 0)
    {
        return false;
    }
    return true;
...
int main()
{
    ...
    int n = alpha.length();
    int answer = 0;
    permute(alpha, 0, n - 1, vt);
    string final_answer = "";
    for (int i = 0; i < vt.size(); ++i)
    {
        if (check(vt[i]) == true)
        {
            final_answer = ADD(final_answer, vt[i]);
        }
    }
    cout << final_answer << "\n";
}