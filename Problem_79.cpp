...
/*###############################################################################################################################################*/
void permute(string str, string out, vector<string> &all_permutations)
{
    if (str.size() == 0)
    {
        all_permutations.push_back(out);
        return;
    }
    for (int i = 0; i < str.size(); i++)
    {
        permute(str.substr(1), out + str[0], all_permutations);
...
int main()
{
    ...
    for (auto x : mp)
    {
        gamma.push_back(x.first);
    }
    vector<string> answers;
    permute(gamma, "", all_permutation);
    for (int i = 0; i < all_permutation.size(); ++i)
    {
        ...
        {
            mp1[all_permutation[i][j]] = j;
        }
        for (int j = 0; j < vt.size(); ++j)
        {
            if ((mp1[vt[j][0]] < mp1[vt[j][1]]) && (mp1[vt[j][1]] < mp1[vt[j][2]]))
            {
                continue;
            }
            else
        ...
        if (flag == true)
        {
            answers.push_back(all_permutation[i]);
        }
    }
    debug(answers);
}